  ...
  useEffect(() => {
    const fetchHoroscope = async () => {
      ...
      try {
        ...
        const response = await fetch(`${baseApiUrl}/numerology/planetary_horoscope`, { method: "POST", body: formData });
  ...
  useEffect(() => {
    const fetchPersonalMonth = async () => {
      ...
      try {
        ...
        formData.append("user_name", username);
        if (dob) formData.append("dob", formatDob(dob));
        const response = await fetch(`${baseApiUrl}/numerology/personal_month`, { method: "POST", body: formData });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchLuckyNumbers = async () => {
      ...
      try {
        ...
        formData.append("user_name", username);
        if (dob) formData.append("dob", formatDob(dob));
        const response = await fetch(`${baseApiUrl}/numerology/lucky_numbers`, { method: "POST", body: formData });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
