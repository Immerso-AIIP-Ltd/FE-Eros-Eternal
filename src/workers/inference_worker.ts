/**
 * Inference Worker - FacePhys Model Inference using LiteRT
 */
...
let Tensor: any = null;
let model: any = null;
let projModel: any = null;
let currentInputTensors: any[] = [];
let loopStep = 0;
...
async function handleInit({ modelBuffer, stateJson, projBuffer }: { modelBuffer: ArrayBuffer; stateJson: any; projBuffer: ArrayBuffer }) {
  // Dynamic import for LiteRT
  ...
  if (currentInputTensors.length > 0) currentInputTensors.forEach((t: any) => t && t.delete());
  currentInputTensors = new Array(INPUT_COUNT);

  const inputsMeta = model.getInputDetails();
  for (let i = 0; i < INPUT_COUNT; i++) {
    const meta = inputsMeta[i];
    const size = meta.shape.reduce((a: number, b: number) => a * b, 1);
...
async function handleRun({ imgData, dtVal, timestamp }: { imgData: Float32Array; dtVal: number; timestamp: number }) {
  ...
  let formattedOutput: Record<string, Float32Array> = {};
  if (!lowPowerMode) {
    const projInputTensors: any[] = [];
  ...
  const resultPayload: InferenceResult = {
    step: loopStep,
    value: mainVal,
    time: end - start,
