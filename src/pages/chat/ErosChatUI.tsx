...
const ErosChatUI: React.FC = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!currentSessionId) {
        const initResponse = await fetch(`http://192.168.1.171:6001/aitools/wellness/v2/chat/spiritual`, {
          method: 'POST',
      ...
      const response = await fetch(`http://192.168.1.171:6001/aitools/wellness/v2/chat/spiritual`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({
          user_id: userId,
