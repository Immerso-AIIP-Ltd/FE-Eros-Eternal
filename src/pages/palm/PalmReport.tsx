...
const PalmReadingReportPage: React.FC = () => {
  ...
  return lines.map((line, index) => {
    ...
    // Bold the part before the first colon (e.g. "Hand Shape:" → <strong>Hand Shape</strong>:)
    const formattedLine = cleanLine
      .replace(/^([^:]+):/, '<strong>$1</strong>:')
      .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');

    return (
      <li key={index} className="mb-3" style={{ listStyleType: 'disc', marginLeft: '20px', lineHeight: '1.8', color: '#374151' }}>
  ...
  const formatArrayItems = (items: string[]) => {
    ...
    return items.map((item, index) => {
      ...
      const cleanItem = item.trim().startsWith("-")
        ...
        : item.trim();
      const finalFormatted = cleanItem.replace(
        /\*\*(.*?)\*\*/g,
        "<strong>$1</strong>",
      );
      ...
      <div className="palm-report-inner flex-grow-1 d-flex flex-column">
        ...
        <div className="palm-report-user d-flex flex-column align-items-center justify-content-center mb-3 mb-md-4">
          <div className="text-center px-2">
            {username && (
              <h2 className="mt-0 mt-md-2 fw-bold" style={{ color: "#1f2937" }}>
                {username}
              </h2>
            )}
            <h6
        ...
        <Container fluid className="palm-report-container px-0">
          <Row className="g-3 g-md-4">
            ...
            {palm_reading_detail.personality_traits &&
              palm_reading_detail.personality_traits.length > 0 && (
                <Col xs={12} md={6}>
                  <Card
            ...
            {palm_reading_detail.life_patterns &&
              palm_reading_detail.life_patterns.length > 0 && (
                <Col xs={12} md={6}>
                  <Card
                    className="palm-report-card h-100 shadow-sm"
                    style={{
            ...
            {palm_reading_detail.career_insights &&
              palm_reading_detail.career_insights.length > 0 && (
                <Col xs={12} md={6}>
                  <Card
                    className="palm-report-card h-100 shadow-sm"
                    style={{
            ...
            {palm_reading_detail.health_observations &&
              palm_reading_detail.health_observations.length > 0 && (
                <Col xs={12} md={6}>
                  <Card
                    className="palm-report-card h-100 shadow-sm"
                    style={{
            ...
            {/* Spiritual Guidance */}
            {palm_reading_detail.spiritual_guidance &&
              palm_reading_detail.spiritual_guidance.length > 0 && (
                <Col xs={12} md={6}>
                  <Card
                    className="palm-report-card h-100 shadow-sm"
                    style={{
            ...
            <Col xs={12}>
              ...
              >
                <Card.Body className="p-4">
                  ...
                  <ul
                    className="list-unstyled mb-0 palm-report-detail"
                    style={{ color: "#374151" }}
                  >
