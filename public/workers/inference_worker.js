...
async function handleInit({ modelBuffer, stateJson, projBuffer }) {
    try {
        console.log('[InferenceWorker] Loading LiteRT from CDN...');
        const litertModule = await import('https://cdn.jsdelivr.net/npm/@litertjs/core@0.2.1/+esm');
        LiteRT = litertModule;
        Tensor = litertModule.Tensor;
        console.log('[InferenceWorker] LiteRT module loaded');

        const originalFetch = self.fetch;
        self.fetch = async (input, init) => {
            if (typeof input === 'string' && input.endsWith('.wasm')) {
                const fileName = input.split('/').pop();
                console.log(`[InferenceWorker] Fetching WASM: ${fileName}`);
                return originalFetch(`${WASM_BASE_URL}${fileName}`, init);
            }
            return originalFetch(input, init);
        };
        
        console.log('[InferenceWorker] Loading LiteRT WASM...');
        await LiteRT.loadLiteRt(WASM_BASE_URL);
        console.log('[InferenceWorker] LiteRT WASM loaded');
        self.fetch = originalFetch;

        console.log('[InferenceWorker] Compiling main model...');
        model = await LiteRT.loadAndCompile(URL.createObjectURL(new Blob([modelBuffer])), { accelerator: 'wasm' });
        console.log('[InferenceWorker] Main model compiled');
        
        console.log('[InferenceWorker] Compiling projection model...');
        projModel = await LiteRT.loadAndCompile(URL.createObjectURL(new Blob([projBuffer])), { accelerator: 'wasm' });
        console.log('[InferenceWorker] Projection model compiled');
    } catch (err) {
        console.error('[InferenceWorker] Init error:', err.message);
        throw err;
    }
